...
- 엑셀 데이터에서 한국어 대사 추출
- 긍/부정 댓글 데이터셋을 학습한 Electra 모델을 사용하여 한국어 대본 각 문장별 평가
- 해당 데이터는 0~1 사이의 값으로 욕설, 악의적 표현 정도를 판단
- 산출된 대본 레이블에서 잘못 평가된 영역을 사람이 수정 

## 3. 데이터셋 학습 방법
- 레이블링된 대본 데이터 + 댓글 데이터를 활용하여 위와 동일한 Electra 모델을 활용하여 학습
- 대본 데이터: 대략 10000 문장, 댓글 데이터: 10000문장
- 학습의 사용된 데이터는 댓글 데이터 + 5배의 대본 데이터 사용
- 학습에 사용되는 multi-sampling의 배수는 Hyperparameter로 사용하여 조절
